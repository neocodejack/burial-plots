...
var nextPreviousBtns = function (type) {
    ...
    if (type == "checkoutNext") {
        showLoader("Please wait..", "info");
        $.ajax({
            url: "/Users/BillingInfoCheck",
            type: "post",
            success: function (res) {
                if (res == "empty") {
                    window.location.href = "/Admin/Users/MyPlan";
                }
                else if (res == "true") {
        ...
        if (confirm("Are you sure you want to remove this item from cart")) {
            ...
            $.ajax({
                ...
                success: function (res) {
                    alert(res);
                    $("#cartCount").text(res);
                    showLoader("<p>Plan is Removed from cart Successfully </p>", "success");
                    setTimeout(function () {
                        hideLoader();
                    }, 4000);
                    //window.location.href = "/Admin/Users/PlanPayment?PlanId=" + id;
                }
            });
        }
    }